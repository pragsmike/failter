...
Failter will **never** overwrite or modify your original prompt template files or your input files.

1.  **Input Directories are Read-Only:** The directories you provide—`inputs/`, `templates/`, and `ground_truth/`—are treated as strictly **read-only** by the Failter application. The program reads from these directories but never writes back to them.

2.  **All Output Goes to a New `results/` Directory:** All output generated by the `experiment` command is written to a **completely separate `results/` directory** that Failter creates inside your experiment folder.

3.  **How Output is Organized:** Inside the `results/` directory, Failter creates further subdirectories for each combination of model and template (e.g., `results/ollama-qwen3_prompt-v1/`). The final output file, which is named after the original input file (e.g., `input1.txt`), is placed inside that subdirectory.

Therefore, there is a clean and safe separation:

*   `my-experiment/templates/my-prompt.md` -> **(READ)**
*   `my-experiment/inputs/my-article.txt` -> **(READ)**
*   ...are processed and a new file is created at...
*   `my-experiment/results/ollama-model_my-prompt/my-article.txt` -> **(WRITE)**
